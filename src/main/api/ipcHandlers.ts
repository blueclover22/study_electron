...
export const setupIpcHandlers = () => {
  ...
  ipcMain.handle('auth:login', async (_event, credentials) => {
    ...
    try {
      ...
      console.log('[Main] Info Field Check:', {
        info: response.data.info,
        isEmpty: !response.data.info,
        isEmptyString: response.data.info === '',
        isObject: typeof response.data.info === 'object',
        keys: response.data.info && typeof response.data.info === 'object' ? Object.keys(response.data.info) : 'N/A'
      });
      ...
      if (isInfoEmpty) {
        ...
        console.log('[Main] Returning failure response:', { success: false, error: errorMessage });
        return {
          success: false,
          error: errorMessage
        };
      ...
      console.log('[Main] Returning success response with user data');
      return { success: true, data: response.data };
    } catch (error: any) {
      console.error('[Main] Login Error:', error);
      // 모든 에러 메시지를 고정 메시지로 통일 (백엔드 메시지 깨짐 방지)
      const errorMessage = '로그인 정보가 올바르지 않습니다.';
      console.error('[Main] Returning error response:', { success: false, error: errorMessage });
      return {
        success: false,
        error: errorMessage
      };
